...
const MyReviews = () => {
    ...
    useEffect(()=>{
        fetch(`https://sidekick-server-soliman-soad.vercel.app/review?email=${user.email}`,{
            headers:{
  ...
  const  handleDelete = id =>{
    fetch(`https://sidekick-server-soliman-soad.vercel.app/review/${id}`,{
      method:'DELETE'
    })
    .then(res => res.json())
    .then(data =>{
         ...
         <div className='min-h-screen'>
            ...
            }
            {
                presentReview.map(present => {
                    return <div key={present._id} className=" mb-5 flex my-10 flex-col bg-gray-800 w-full m-full p-6 mx-auto divide-y rounded-md divide-gray-700 text-white shadow-lg border-2">
